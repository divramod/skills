...
| [plan](./skills/plan/SKILL.md) | Lean planning: one markdown file per plan in `plans/<NNNN>-<slug>.md` (goal, steps with a done-when check and status, decisions), `plans/CURRENT_PLAN` names the active one for the statusline. `/plan new`, `/plan`, `/plan n` (next, offers `/grill` first), `/plan d` (done), `/plan h` (help); execution uses plan mode, `/goal` and subagents. |
