...
| [plan](./skills/plan/SKILL.md) | Lean planning: one markdown file per plan in `docs/plans/<NNNN>-<slug>.md` (goal, steps with a done-when check and status, decisions), `docs/plans/CURRENT_PLAN` names the active one for the statusline. `/plan new`, `/plan`, `/plan next` (offers `/grill-me` first), `/plan done`; execution uses plan mode, `/goal` and subagents. |
| [grill-me](./skills/grill-me/SKILL.md) | Relentless interview about a plan or idea until every branch of its design tree is resolved: rounds through the question tool with a recommended answer each, facts looked up instead of asked, decisions written to the plan and `docs/intent.md` so nothing is asked twice. Adapted from [Matt Pocock's grill-me](https://github.com/mattpocock/skills) (MIT). |
