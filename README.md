...
| Skill | What it does |
|---|---|
| [handoff](./skills/handoff/SKILL.md) | Write or refresh `docs/handoff.md` so a fresh session can continue: goal, a link to the plan and its current step, what is done, the next tasks with a done-when check, traps, open decisions and the prompt to start with; commits only that file. `/handoff continue` reads it back and carries on. |
