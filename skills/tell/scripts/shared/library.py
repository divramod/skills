...
let st=DEF;
try{st=Object.assign({},DEF,JSON.parse(localStorage.getItem('tell-lib')||localStorage.getItem('tell-me-lib')||localStorage.getItem('dm-lib')||'{}'));st.dir=Object.assign({},DEF.dir,st.dir);}catch(e){}
if(!Array.isArray(st.types))st.types=[];  // a stale or hand-edited value
const save=()=>{try{localStorage.setItem('tell-lib',JSON.stringify(st))}catch(e){}};
...
try{const w=+(localStorage.getItem('tell-lib-width')||localStorage.getItem('tell-me-lib-width')||localStorage.getItem('dm-lib-width'));if(w)setW(w);}catch(e){}
const saveW=()=>{try{localStorage.setItem('tell-lib-width',aside.getBoundingClientRect().width|0)}catch(e){}};
...
handle.addEventListener('dblclick',()=>{document.documentElement.style.removeProperty('--side');try{for(const k of ['tell-lib-width','tell-me-lib-width','dm-lib-width'])localStorage.removeItem(k)}catch(e){}});
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--root", type=Path, help="library root (default: $TELL_ROOT or ~/skills/tell)")
    ...
    if args.root:
        os.environ["TELL_ROOT"] = str(args.root)
    root = library_root().resolve()
    if args.open_last:
