"""Shared helpers for the tell scripts. Stdlib only.

Library layout: <root>/<kind>/.../ per source (see dir_for), each folder with summary.md,
metadata.json and the content file; <root>/library.js indexes them all. Root: $TELL_ROOT (old name:
$TELL_ME_ROOT), else the parent of $DM_SUMMARIZE_VIDEO_ROOT (the old videos-only root), else ~/skills/tell (a
skill's own data lives under ~/skills/<skill-name>/). A library still at an old default (~/skills/tell-me,
~/me/summaries) stops the scripts with the command that moves it. The skill was called tell-me, and before that
dm-summarize-video: every setting reads its old names too (env()).
"""
...
def log(msg: str) -> None:
    print(f"[tell] {msg}", file=sys.stderr)
...
def env(*names: str) -> str | None:
    """The first of these environment variables that is set: the current name, then the old ones."""
    return next((os.environ[n] for n in names if os.environ.get(n)), None)


def browser_cookies() -> str | None:
    """The browser yt-dlp reads cookies from: $TELL_BROWSER (old names: $TELL_ME_BROWSER,
    $DM_SUMMARIZE_VIDEO_BROWSER)."""
    return env("TELL_BROWSER", "TELL_ME_BROWSER", "DM_SUMMARIZE_VIDEO_BROWSER")


def library_root() -> Path:
    if root := env("TELL_ROOT", "TELL_ME_ROOT"):
    ...
    if old:  # the videos-only root of earlier versions: <root>/videos
        ...
        if videos.name != KIND_DIRS["video"]:
            raise SkillError(f"DM_SUMMARIZE_VIDEO_ROOT={old} is not a folder named 'videos'. tell now keeps every "
                             f"source under one root: set TELL_ROOT to that root and move the videos into "
    ...
    if not root.exists():
        for old in legacy_roots():
            if (old / "library.js").exists():
                raise SkillError(f"the tell library moved from {old} to {root}. Move it (stop a running video "
                                 f"download first): python3 {SCRIPTS_DIR / 'serve_library.py'} --stop && mkdir -p "
                                 f"{root.parent} && mv {old} {root}   (or keep it there: TELL_ROOT={old})")
    return root


def default_root() -> Path:
    return Path.home() / "skills" / "tell"
...
def legacy_roots() -> tuple[Path, ...]:
    ...
    return Path.home() / "skills" / "tell-me", Path.home() / "me" / "summaries"
