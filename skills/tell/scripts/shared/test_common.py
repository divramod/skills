...
class TestNaming(unittest.TestCase):
    ...
    def test_default_root_and_env_override(self):
        with tempfile.TemporaryDirectory() as home, mock.patch.dict(os.environ, {"HOME": home}, clear=True):
            self.assertEqual(library_root(), Path(home) / "skills" / "tell")
            self.assertEqual(source_root("video"), Path(home) / "skills" / "tell" / "videos")
        with mock.patch.dict(os.environ, {"TELL_ROOT": "/tmp/x", "DM_SUMMARIZE_VIDEO_ROOT": "/tmp/y/videos"}):
            self.assertEqual(library_root(), Path("/tmp/x"))
        with mock.patch.dict(os.environ, {"TELL_ME_ROOT": "/tmp/m"}, clear=True):
            self.assertEqual(library_root(), Path("/tmp/m"))  # the name before the rename to tell
        with mock.patch.dict(os.environ, {"TELL_ROOT": "/tmp/x", "TELL_ME_ROOT": "/tmp/m"}, clear=True):
            self.assertEqual(library_root(), Path("/tmp/x"))
        ...
        with mock.patch.dict(os.environ, {"DM_SUMMARIZE_VIDEO_ROOT": "/tmp/yt-notes"}, clear=True):
            with self.assertRaisesRegex(SkillError, "set TELL_ROOT"):
                library_root()  # its parent may be ~: never guess

    def test_a_library_at_an_old_default_stops_with_the_move_command(self):
        for old_rel in ("skills/tell-me", "me/summaries"):
            with self.subTest(old_rel), tempfile.TemporaryDirectory() as home, \
                    mock.patch.dict(os.environ, {"HOME": home}, clear=True):
                old, new = Path(home) / old_rel, Path(home) / "skills" / "tell"
                old.mkdir(parents=True)
                self.assertEqual(library_root(), new)  # an empty old folder: no library
                (old / "library.js").write_text("")
                with self.assertRaisesRegex(SkillError, re.escape(f"mv {old} {new} ")):
                    library_root()
                with mock.patch.dict(os.environ, {"TELL_ROOT": str(old)}):
                    self.assertEqual(library_root(), old)  # kept there on purpose
                new.mkdir(parents=True, exist_ok=True)
                self.assertEqual(library_root(), new)  # moved (or a new one started)

    def test_browser_cookies_new_name_first_then_the_old_ones(self):
        ...
        with mock.patch.dict(os.environ, {"TELL_ME_BROWSER": "safari", "DM_SUMMARIZE_VIDEO_BROWSER": "firefox"},
                             clear=True):
            self.assertEqual(browser_cookies(), "safari")
        with mock.patch.dict(os.environ, {"TELL_BROWSER": "chrome", "DM_SUMMARIZE_VIDEO_BROWSER": "firefox"}):
