...
class TestLibrary(unittest.TestCase):
    def setUp(self):
        ...
        self.env = mock.patch.dict(os.environ, {"TELL_ROOT": str(self.root)})
        self.env.start()
...
class TestApi(unittest.TestCase):
    ...
    def test_unknown_or_escaping_paths(self):
        ...
        self.assertEqual(self.call("/api/download", {"path": "../x"}, {"X-Tell": "1"})[0], 404)
    ...
    def test_download_needs_header_and_same_host(self):
        ...
        self.assertEqual(self.call("/api/download", {"path": "videos/youtube/chan/zeta-talk"},
                                   {"X-Tell": "1", "Origin": "https://evil.example"})[0], 403)
    ...
    def test_download_starts_best_quality_background_job(self):
        ...
        with mock.patch("serve_library.video_cli", side_effect=fake):
            code, _ = self.call("/api/download", {"path": "videos/youtube/chan/zeta-talk"}, {"X-Tell": "1"})
    ...
    def test_pages_rendered_before_the_renames_still_send_the_old_headers(self):
        for header in ("X-Tell-Me", "X-DM-Summarize"):
            with mock.patch("serve_library.video_cli", return_value=(0, {"status": "running"})):
                code, _ = self.call("/api/download", {"path": "videos/youtube/chan/zeta-talk"}, {header: "1"})
            self.assertEqual(code, 202, header)
        self.assertEqual(self.call("/api/download", {"path": "videos/youtube/chan/zeta-talk"}, {"X-Tell": "0"})[0], 403)
    ...
    def test_an_x_posts_video_downloads_from_the_post_with_its_item(self):
        ...
        with mock.patch("serve_library.video_cli", side_effect=fake):
            ...
            self.assertEqual(self.call("/api/download", {"path": rel}, {"X-Tell": "1"})[0], 202)
    ...
    def test_video_cli_failures_become_json_errors(self):
        ...
        with mock.patch("serve_library.video_cli", return_value=(1, "boom")):
            ...
            code, data = self.call("/api/download", {"path": "videos/youtube/chan/zeta-talk"}, {"X-Tell": "1"})
    ...
    def test_delete_video_endpoint(self):
        ...
        code, data = self.call("/api/delete-video", {"path": "videos/youtube/chan/zeta-talk"}, {"X-Tell": "1"})
