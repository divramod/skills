...
from _common import SkillError, find_by_id, log, read_json, unique_dir, update_json
...
def fetch_issue(gh, r: dict) -> dict:
    ...
    if kind == "pull":
        ...
        facts |= pr
        comments += review
    comments.sort(key=lambda c: c.get("date") or "")
...
def fetch_discussion(gh, r: dict) -> dict:
    ...
    while True:
        ...
        if not d:
            raise SkillError(f"no discussion #{r['number']} in {r['repo']}")
        page = d["comments"]
        nodes += page["nodes"]
    ...
    facts = {"kind": "discussion", "number": r["number"], "state": "closed" if d.get("closed") else "open",
             ...
             "reactions": d.get("upvoteCount") or None}
...
def prepare_thread(gh, r: dict, refresh: bool) -> tuple[Path, dict, bool]:
    """(folder, metadata, reused) of an issue, pull request or discussion."""
    ...
    extras = {k: v for k, v in (f | {"comments": len(t["comments"]), "repo": r["repo"], "api": gh.mode,
                                     "files": len(t["files"]) or None}).items() if v is not None}
    meta = {
        "source": "github", "id": r["id"], "url": t["url"] or r["url"], "title": t["title"] or r["id"],
