...
Requires: ffmpeg. The folder must contain video.<ext> (prepare_video.py -d).
"""
...
def extract(folder: Path, max_frames: int = 40, threshold: float = 0.3, min_frames: int = 4) -> Path:
    ...
    if not video:
        raise SkillError(f"no video.<ext> in {folder}; run prepare_video.py with --download first")
